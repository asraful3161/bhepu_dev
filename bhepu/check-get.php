...
// Division Check && District Check  //

if(!empty($_GET['division_id'])) {
	if($_GET['division_id'] == 'All Division') {
		$a_division = '';
		$b_division = '';
	} else {
		$a_division = '&division_id=';
		$b_division = $_GET['division_id'];
	}	
} else {
	$a_division = '';
	$b_division = '';
...
if(!empty($_GET['district_id'])) {
	if($_GET['district_id'] == 'All District') {
		$a_district = '';
		$b_district = '';	
	} else {
		if($_GET['district_id'] == 'All Divisions') {
			$a_district = '';
			$b_district = '';
		} else {
			$a_district = '&district_id=';
			$b_district = $_GET['district_id'];
		}
	}
...
$all_gets = '';
$all_gets = $a_brand.$b_brand.$a_model.$b_model.$a_division.$b_division.$a_district.$b_district.$a_car_condition.$b_car_condition.$a_price_range.$b_price_range.$a_car_category_id.$b_car_category_id.$a_body_type_id.$b_body_type_id.$a_fuel_type_id.$b_fuel_type_id.$a_transmission_type_id.$b_transmission_type_id.$a_year.$b_year.$a_mileage_start.$b_mileage_start.$a_mileage_end.$b_mileage_end.$a_country.$b_country;
