...
<div class="widget-item">
	<h3>Division</h3>
	<select data-placeholder="Choose Division" class="form-control division" name="division_id" onchange="this.form.submit()">
		...
		foreach ($result as $row) {
			?>			
			<option value="<?php echo $row['division_id']; ?>" <?php if(isset($_GET['division_id']))  {if($_GET['division_id']==$row['division_id']) {echo 'selected';}} ?>><?php echo $row['division_name']; ?></option>
			<?php
		}
		?>
	</select>
...
<div class="widget-item">
	...
	<select data-placeholder="Choose district" class="form-control district" name="district_id" style="height: 38px;" onchange="this.form.submit()">
		<option value="">Choose District</option>
	</select>
	<?php else: ?>
	<select data-placeholder="Choose District" class="form-control chosen-select" name="district_id" onchange="this.form.submit()">
		<option>All District</option>
		<?php
		$statement = $pdo->prepare("SELECT * FROM tbl_district WHERE division_id=? ORDER BY district_name ASC");
		$statement->execute(array($_GET['division_id']));
		$result = $statement->fetchAll(PDO::FETCH_ASSOC);
		foreach ($result as $row) {
			?>			
			<option value="<?php echo $row['district_id']; ?>" <?php if(isset($_GET['district_id']))  {if($_GET['district_id']==$row['district_id']) {echo 'selected';}} ?>><?php echo $row['district_name']; ?></option>
			<?php
		}
		?>
	</select>
...
<div class="widget-item">
	...
	<select data-placeholder="Choose Condition" class="form-control chosen-select" name="car_condition" onchange="this.form.submit()">
		...
		<option value="Old Car" <?php if(isset($_GET['car_condition']))  {if($_GET['car_condition']=='Used Car') {echo 'selected';}} ?>>Used Car</option>
	</select>
</div>
